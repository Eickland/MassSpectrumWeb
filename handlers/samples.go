    ...
    for _, file := range files {
        ...
        if existingSample, ok := newSamples[nameWithoutExt]; ok {
            ...
            existingSample.Graphs = append(existingSample.Graphs, models.Graph{
                Name:     fileName,
                // **ИСПРАВЛЕНО**: Формируем правильный URL для доступа к файлу
                FilePath: "/data/graphs/" + fileName,
                FileType: strings.TrimPrefix(ext, "."),
                IsMain:   len(existingSample.Graphs) == 0, // Первый график - основной
            })
